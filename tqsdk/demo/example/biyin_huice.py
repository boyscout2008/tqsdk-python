...
'''
趋势型短线交易信号 - 夸父逼阴
1. 主空或多转空品种一波空已经启动，次日再次做中大阴；或1~2小中继后继续做空
...
#STEP2：策略执行log
logger.info("Starting biyin strategy for: %s, actually year: %s with zhicheng1: %s, zhicheng2: %s"%(SYMBOL, YEAR, ZC1, ZC2))
...
while True:
    ...
    if api.is_changing(klines):
        ...
        #logger.info("biyin date: %s, adjust interval: %d" %(trading_date, 20 - index - 1))
        if isBiyin:
