...
logger.info("start zhendang_short_model_2 daily strategy for %s in %d.%d.%d!"%(SYMBOL, YEAR, MONTH, DAY))
...
while True:
    ...
    if api.is_changing(klines[-1], "datetime"):
        ...
        #logger.info("CURRENT PRICES:  close = %f, vwap = %f, day_open = %f at %d!" % (df["close"].iloc[-1], df["vwap"].iloc[-1], df["open"].iloc[0], len(df)))
