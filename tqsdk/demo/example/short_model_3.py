...
'''
日内交易信号 - 震荡趋势大背离空或滞涨后做空模型2_1 （0+- 0- 0|0-）
1. 震荡一波多后分时之上阻力位做空
2  它有两种形态：一波蓄势不足多大背离局部滞涨直接开空，或者蓄势充足或背离不多滞涨后做空 --- PANZHONGGAO_SHORT
...
while True:
    ...
    if api.is_changing(klines[-1], "datetime"):
        ...
        if len(df) - last_kong_index > 20 and df["close"].iloc[-1] > df["vwap"].iloc[-1] *1.002 \
            and df["close"].iloc[-1] > YALIWEI*0.992 and (df_zz["close"]>df_zz["vwap"]*0.995).all():
            current_volume += -1*TARGET_VOLUME
            traded_volume += TARGET_VOLUME
            target_pos.set_target_volume(current_volume)
            last_kong_index = len(df)
